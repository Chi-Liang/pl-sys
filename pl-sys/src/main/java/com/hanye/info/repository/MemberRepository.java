...
public interface MemberRepository extends CrudRepository<Member, String> {
	...
	String sql = "select  m.mid, m.name, m.tel,m.email,m.address,m.create_date,m.update_date,m.free_or_paid,mp.points,m.which_group,"
			+ " group_concat(c.name order by c.cid asc separator ';') as categoryNames"
			+ " from member m inner join member_point mp on m.mid = mp.mid left join member_categroy mc on m.mid = mc.member_id left join category c on c.cid = mc.category_id"
			+ " group by m.mid";
